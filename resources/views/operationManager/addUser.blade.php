...
@section('page-content')
	...
	@if($form_action == '/updateUserByOperationManager')
	<div class = "col-md-12">
		<h4>User Roles</h4>
		<hr />
	</div>
		...
		@if(isset($roles_selected['district_operation_id']))
			...
			<div class="col-sm-7">
				<?php $arrayDistrictOperation = array(); ?>
					...
					@empty
					@endforelse
				{{Form::select('district_operation'.$x, $arrayDistrictOperation,$roles_selected['district_operation_id'],array('class'=>'form-control'))}}
			</div>
		@endif	
	...
	jQuery(function($) {
		...
		$(document).on('change','#role_type',function(){
			...
			if(roleType != "" || roleType != null){
				$.ajax({
					url:'/operationManager/getCountryByRoleType',
					type:'POST',
		...
		$(document).on('change','#country',function(){
			...
			if(country != 0){
				...
				$.ajax({
					url:'/operationManager/getDistrictOperationByCountryId',
					type:'POST',
...
/*	var row_counter = $('#row_counter').html();
	...
	if(form_action == 'http://localhost:8000/updateUserByOperationManager'){	
	 var x = 0;
	}
	else{
		x = 1;
