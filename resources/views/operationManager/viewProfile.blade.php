...
@section('breadcrumb-section')
	...
	<li class="active">Operation Manager Dashboard</li>
	@endsection
...
<script type="text/javascript">
      jQuery(function($) {
        ...
        $(document).on('focusout','#oldPassword',function(){
          ...
          $.ajax({
            type:'POST',
            url:'/operationManager/verifyPassword',
            data:{
              userId:userId,
      ...
      $(document).on('click','.save-password',function(){
        ...
        $.ajax({
            type:'POST',
            url:'/operationManager/changePassword',
            data:{
              userId:userId,
              oldPassword:oldPassword,
        ...
        $( "#changePassword" ).on('click', function(e) {
         ...
         $( "#assignProjects" ).on('click', function(e) {
          e.preventDefault();
          var roleUserId = $(this).val();
          $.ajax({
            type:'POST',
            url:'/operationManager/userProjects',
            data:{roleUserId:roleUserId,'_token':"{{csrf_token()}}"},
            success:function(data){
                
            var dialog = $( "#dialog-assign-project" ).removeClass('hide').dialog({
            modal: true,
            width: '500',
            title: "<div class='widget-header widget-header-small'><h4 class='smaller'><i class='ace-icon fa fa-key'></i> Select Projects</h4></div>",
            title_html: true,
            buttons: [ 
              {
                text: "Cancel",
        ...
        $(document).on('click','.assign-projects',function(){
           var i = 0;
             var districtOperationProjectId = [];
             var roleUserId = $('#assignProjects').val();
             $('.checkbox:checked').each(function () {
                 districtOperationProjectId[i++] = $(this).data('district-operation-project-id')
             });      
             $.ajax({
              url:'/operationManager/assignProjects',
              type:'POST',
              data:{
          ...
          e.preventDefault();
          var projectUserId = $(this).attr('href');
          var roleUserId = $('#assignProjects').val();
          $.ajax({
            url:'/operationManager/deleteUserProject',
                type:'POST',
                data:{
