...
<div class="row">
    <div class="col-sm-12">
        ...
        @endif
        {!! Form::open(array('url'=>'countryManager/saveDocumentTemplate', 'method'=>'POST' , 'class'=>'form-horizontal' , 'id'=>'template-form')) !!}
              	...
              	<div class="modal-content">
              		<div class="modal-body">
              			...
              			<div class="bootbox-body">
                            ...
                            <i class="ace-icon fa fa-exclamation-triangle bigger-120"></i> Please Given Section Priority
                            </h3>
                    </div>
              		</div>
            ...
            <div class="modal-dialog">
              	<div class="modal-content">
              		<div class="modal-body">
              			...
              			<div class="bootbox-body">
                            <h3 class="text-danger text-center">
                                <i class="ace-icon fa fa-exclamation-triangle bigger-120"></i> Please Select Questions
                            </h3>
                        </div>
              		</div>
              		<div class="modal-footer background-blue"></div>
...
<script>
	...
	$(document).ready(function(){
        ...
        $(document).on('change' ,'input[name=checkedTemplate]:checked' ,function(){
        	...
        	$.ajax({
		            url:"{{ url('countryManager/getGeneralDiv') }}",
		            type:"GET",
		            //dataType: 'json',
		            beforeSend:function(){
                        $('#processing_div').show();
    	...
    	function getSectionQuestions(sectionID =null){
    		...
    		if(sectionID){
    			$.ajax({
		                url:"{{ url('countryManager/getSectionWithQuestionsBySectionId') }}",
		                type:"GET",
		                //dataType: 'json',
		                beforeSend:function(){
                            $('#processing_div').show();
		...
		$(document).on('change' , '#predefined-template-id' , function(){
			...
			if(document_template_id != ''){
				...
				$.ajax({
		            url:"{{ url('countryManager/getPredefinedTemplateByTemplateId') }}",
		            type:"GET",
		            //dataType: 'json',
		            beforeSend:function(){
                        $('#processing_div').show();
