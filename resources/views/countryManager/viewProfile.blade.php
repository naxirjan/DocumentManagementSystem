...
@section('breadcrumb-section')
	...
	<li class="active">Country Manager Dashboard</li>
	@endsection
        ...
        <div class="width-80 label label-info label-xlg arrowed-in arrowed-in-right">
            <div class="inline position-relative">
              <div class="user-title-label">
                ...
                &nbsp;
                <span class="white"><b>{{session()->get('current_active_role_type')}} ({{session()->get('current_cont_name')}})</b></span>
              </div>
            </div>
        </div>
        @endif
              ...
              <div class="widget-main no-padding">
                  <table class="table table-striped table-bordered table-hover">
                    <thead class="thin-border-bottom">
                      <tr>
                        <th><i class="ace-icon fa fa-key"></i>Role</th>
                        <th><i class="ace-icon fa fa-flag"></i>Country</th>
                        <th class=""><i class="ace-icon fa fa-map-marker"></i>Locations</th>
                      </tr>
                    </thead>
                            ...
                            @foreach($roles['dop'] as $dop)
                              @if($dop['role_id'] == 3 || $dop['role_id'] == 4 || $dop['role_id'] == 5)
                                <h5>
                                  ...
                                  @if($dop['role_id'] == 5 && $dop['role_status'] == "Active")
...
jQuery(function($) {
    ...
    $(document).on('focusout','#oldPassword',function(){
          ...
          $.ajax({
            type:'POST',
            url:'/countryManager/verifyPassword',
            data:{
              userId:userId,
    ...
    $(document).on('click','.save-password',function(){
        ...
        $.ajax({
            type:'POST',
            url:'/countryManager/changePassword',
            data:{
              userId:userId,
              oldPassword:oldPassword,
    ...
    $(document).on('click',".btnAssignProjects",function(){
      ...
      if(roleUserId && dopId){
        $.ajax({
            type:'POST',
            url:'/countryManager/userProjects',
    ...
    $(document).on('click','.assign-projects',function(){
      ...
      if(districtOperationProjectId.length > 0){
        $.ajax({
              url:'/countryManager/assignProjects',
              type:'POST',
              data:{
              ...
              success:function(data){
                  if(data != 'Project Not Assigned'){
                    ...
                    setTimeout(function(){
                      window.location.href ="/countryManager/profile/<?php echo Request::segment(3); ?>";  
                    },2000);
    ...
    $(document).on('click','.deleteUserProject',function(e){
      ...
      $.ajax({
          url:'/countryManager/deleteUserProject',
            type:'POST',
            data:{
