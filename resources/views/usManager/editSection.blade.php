...
<div class="row">
	<div class="col-sm-12">
{!! Form::open(array('url'=>'usManager/updateSection', 'method'=>'POST' , 'class'=>'form-horizontal' , 'id'=>'section-form')) !!}
<div>
	<label for="form-field-8"><b>Section Title <span class="text-danger">(*)</span></b></label>
	<input type="text" name="section_title" class="form-control" id="form-field-8" value="{{$sectionQuestions[0]['section_title']}}" placeholder="Enter Section Title" />
	<div id="section-title-error" style="display: none">
		<div class="space-4"></div>
		<span  class="col-md-12 col-sm-12 col-xs-12 label label-xs label-danger arrowed arrowed-right"> The Section Field Is Required</span>
...
<div>
	...
	<textarea name="section_description" class="form-control" id="form-field-8" placeholder="Enter Section Description">{{$sectionQuestions[0]['section_description']}}</textarea>
	<div id="section-description-error" style="display: none">
		<div class="space-4"></div>
		<span  class="col-md-12 col-sm-12 col-xs-12 label label-xs label-danger arrowed arrowed-right"> The Section Description Field Is Required</span>
...
<div>
	...
	<input name='section_status' class='ace ace-switch ace-switch-6' type='checkbox' @php echo ($sectionQuestions[0]['status'] == 'Active')?'checked':''; @endphp value='@php echo$sectionQuestions[0]['status']; @endphp'>
	...
	<div class="widget-box widget-color-blue light-border">
		<div class="widget-header">
			...
			<div class="widget-toolbar">
				<h5 class="badge badge-danger" id="question_pool_count"><?php echo count($remainingActiveQuestions); ?></h5>
			</div>
		</div>

		<div class="widget-body">
			@if($remainingActiveQuestions)
			...
			<div class="widget-main padding-6" id="scroll_bar" style="overflow-y:auto;height:410px">
				<ul class="list-unstyled spaced2 item-list" id="all-questions">
					...
					@forelse($remainingActiveQuestions as $question)
...
<div class="col-md-8 col-sm-8">
	<div class="widget-box widget-color-blue light-border">
		<div class="widget-header">
			...
			<div class="widget-toolbar">
				<h5 class="badge badge-danger" id="section_question_count">{{count($sectionQuestions[0]['questions'])}}</h5>
			</div>
		</div>
		...
		<div class="widget-body">
			<div class="widget-main padding-6">
				<div class="widget-main padding-6" id="scroll_bar" style="overflow-y:auto;height:410px">
				<li id="div_remove_all" style="display:none">
				...
				<ul class="list-unstyled spaced2 item-list ui-sortable-handle" id="section-questions">
                	@php $count = 1 @endphp
                	@foreach($sectionQuestions[0]['questions'] as $secQuestion)
                		<li class='item-green clearfix ui-sortable-handle' data-order="{{$secQuestion['question_id']}}" id="questions-row{{$secQuestion['question_id']}}" style='cursor:pointer;'>
							<div>
							<h5><span><b class='question_serial'>Question {{ $count++ }}</b></span>
							</h5>
							<div class='col-md-12 col-sm-12 col-xs-12'><p>{{$secQuestion['question']}}</p></div><br/>
							<div class='hr hr-24 dotted hr-double'></div>
							<label class='pull-right'>
							<span><b>Status: </b></span><input name='question_status' class='ace ace-switch ace-switch-6' type='checkbox' @php echo ($secQuestion['status'] == 'Active')?'checked':'' @endphp value='1'><span class='lbl middle'></span>
							</label>
							<input class='section-question' type='hidden' question_id="{{$secQuestion['question_id']}}" question_name='title' name='section_questions[]' value="{{$secQuestion['question_id']}}"/>
							</div>
						</li>
                	@endforeach
                </ul>
...
<div class="col-md-offset-6 col-md-6 col-sm-offset-6 col-sm-6" id="div-btn">
...
<input type="hidden" name="section_id" value="{{$sectionQuestions[0]['section_id']}}" id="section-id" />
{!! Form::close() !!}
...
<script>
	$(document).ready(function(){
			...
			*/
            ...
            $('ul#all-questions>#questions-row'+$(this).attr('question_id')).remove();
       		...
       		if($("#all-questions input[name='questions']").length == 0){
       			...
       			$('#add_all').css('visibility','hidden');//hide();
       			$("#search_questions").hide();
       		}
		...
		$(document).on('click','.remove-individual',function() {
            ...
            if($("#section-questions input[name='section_questions[]']").length == @php echo count($sectionQuestions[0]['questions'])  @endphp){
       				/*To Hide Remove All*/
       				$("#div_remove_all").hide();
	        	...
	        	if(section_title && section_description && $('ul#section-questions li').length>=1)
	          	{
	          		...
	          		/*jQuery Ajax*/
	                $.ajax({
		                url:"{{url('usManager/updateSection')}}",
		                type:"POST",
		                dataType: 'json',
		                beforeSend:function(){
		                	$('#div-btn').hide();
			                ...
			                success:function(data){
			                	...
			                	if(data.message == 'Yes'){
			                		...
			                		setTimeout(function(){
			                			...
			                			$('#div-btn').show();
			                			window.location.href = "{{url('usManager/viewSectionDetail')}}/"+section_id;
			                		},5000);
			                	}else{
			                		$('#div-message>#loading').hide();
			                		...
			                		setTimeout(function(){
			                			$('#div-message>#error-message-not').hide();
			                			$('#div-btn').show();
			                			window.location.href = "{{url('usManager/viewSection')}}";
			                		},5000);
			                	}
			                }             
	                });
        ...
        function removeAll(){
            ...
            */
            ...
            var questions = [];
         	...
         	$.each($("#section-questions input[name='section_questions[]']:not('.section-question')"), function(){
